...
class PublishGuideServiceProvider_Test extends \WP_UnitTestCase {
	...
	/**
	 * Test is_excluded_page returns correct values for excluded and non-excluded pages.
	 */
	public function test_is_excluded_page() {
		$reflection = new \ReflectionClass( $this->instance );
		$method     = $reflection->getMethod( 'is_excluded_page' );
		$method->setAccessible( true );
		global $pagenow;
		$pagenow = 'admin.php';

		$excluded = array( 'wc-admin', 'other-page' );

		// Excluded page.
		$_GET['page'] = 'wc-admin';

		$result_true = $method->invoke( $this->instance, $excluded );
		$this->assertTrue( $result_true );

		// Non-excluded page.
		$_GET['page'] = 'not-excluded';
		$result_false = $method->invoke( $this->instance, $excluded );
		$this->assertFalse( $result_false );
	}

	/**
	 * Test is_excluded_post_type returns correct values for excluded and non-excluded post types.
	 */
	public function test_is_excluded_post_type() {
		$reflection = new \ReflectionClass( $this->instance );
		$method     = $reflection->getMethod( 'is_excluded_post_type' );
		$method->setAccessible( true );
		global $pagenow;
		$pagenow = 'edit.php';

		$excluded = array( 'product', 'shop_order' );
		...
		// Non-excluded post type.
		$_GET['post_type'] = 'not-excluded';
		$result_false      = $method->invoke( $this->instance, $excluded );
		$this->assertFalse( $result_false );
	}
}
